...
export function LocationForm({ property }: ComponentStepProps): ReactElement {
  const { currentUser } = useAppContext()
  const tLocationForm = useTranslations('host.property.locationForm')
  const {
    steps,
    currentStep,
    updateStep,
  ...
  const form = useForm<LocationFormType>({
    ...
    defaultValues: {
      ...
      additionalInfo: property?.location?.additionalInfo ?? '',
      aptInfo: property?.location?.aptInfo ?? '',
  ...
  async function onSubmit(
    ...
    try {
      await axios.post(
        `/api/host/${currentUser?.id}/properties/${propertyId}/location`,
        data,
      )
      return true
    } catch (error) {
