...
export function StructureForm({ property }: ComponentStepProps): ReactElement {
  const { currentUser } = useAppContext()
  const tStructureForm = useTranslations('host.property.structureForm')
  ...
  async function onSubmit(
    ...
    try {
      await axios.post(
        `/api/host/${currentUser?.id}/properties/${propertyId}/structure`,
        data,
      )
      return true
    } catch (error) {
  ...
  return (
    <Box display="flex" flex-direction="col" gap={11}>
      <HeadingGroup title={tStructureForm('heading.title')} />
