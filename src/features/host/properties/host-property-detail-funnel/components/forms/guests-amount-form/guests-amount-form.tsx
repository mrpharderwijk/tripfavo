...
export function GuestsAmountForm({
  ...
  const tGuestsAmountForm = useTranslations('host.property.guestsAmountForm')
  const {
    steps,
    currentStep,
    updateStep,
  ...
  const form = useForm<z.infer<typeof GuestsAmountFormSchema>>({
    ...
    mode: 'onChange',
    defaultValues: {
      maxGuests: property?.guestsAmount?.maxGuests ?? 1,
      adults: property?.guestsAmount?.adults ?? 1,
      children: property?.guestsAmount?.children ?? 0,
      infants: property?.guestsAmount?.infants ?? 0,
      pets: property?.guestsAmount?.pets ?? 0,
    },
  })
  const {
    formState: { errors },
  ...
  async function onSubmit(
    ...
    try {
      await axios.post(
        `/api/host/${currentUser?.id}/properties/${propertyId}/guests-amount`,
        data,
      )
      return true
    } catch (error) {
