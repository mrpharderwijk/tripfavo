...
export function PriceForm({ property }: ComponentStepProps): ReactElement {
  const { currentUser } = useAppContext()
  const tPriceForm = useTranslations('host.property.priceForm')
    ...
    defaultValues: {
      priceDetails: [
        ...
        },
        {
          ...
          price:
            property?.priceDetails?.find((pd) => pd.type === PriceType.DEPOSIT)
              ?.price ?? 0.0,
        },
      ],
    },
  ...
  async function onSubmit(
    ...
    try {
      await axios.post(
        `/api/host/${currentUser?.id}/properties/${propertyId}/price-details`,
        data,
      )
      return true
    } catch (error) {
