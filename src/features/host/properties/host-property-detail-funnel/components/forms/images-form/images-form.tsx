...
import { PropertyFormImage } from '@/components/organisms/property-form-image/property-form-image'
...
export function ImagesForm({ property }: ComponentStepProps): ReactElement {
  const { currentUser } = useAppContext()
  const tPropertyImagesForm = useTranslations('host.property.imagesForm')
  const [prismaError, setPrismaError] = useState<string | null>(null)
  const [utError, setUtError] = useState<string | null>(null)
  const { updateStep, propertyId, setIsLoading } = useHostContext()
  const [images, setImages] = useState<
    Omit<PropertyImage, 'propertyId' | 'userId' | 'updatedAt' | 'roomId'>[]
  >(property?.images || [])
  ...
  async function onSubmit(
    ...
    try {
      await axios.post(
        `/api/host/${currentUser?.id}/properties/${propertyId}/images`,
        {
          files: data.images,
        },
      )
  ...
  function handleOnBeforeUploadBegin(files: File[]): File[] {
    ...
    return files.map(
      (f) => new File([f], `${propertyId}-${f.name}`, { type: f.type }),
    )
  }
  ...
  return (
    <Box display="flex" flex-direction="col" gap={11}>
      <HeadingGroup
        title={tPropertyImagesForm('heading.title')}
        subtitle={tPropertyImagesForm('heading.subtitle', {
