...
export async function POST(request: NextRequest, { params }: HostListingParams): Promise<NextResponse> {
  ...
  if (!amenities) {
    return NextResponse.json({ message: 'Amenities are required' }, { status: 400 })
  }
  ...
  try {
    ...
    return NextResponse.json({ message: 'Amenities updated successfully' }, { status: 200 })
  } catch (error) {
    console.error(error)
