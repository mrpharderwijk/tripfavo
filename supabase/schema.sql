...
CREATE TYPE property_status AS ENUM ('DRAFT', 'IN_REVIEW', 'PUBLISHED');
...
-- Property guests amount table
CREATE TABLE property_guests_amount (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  max_guests INTEGER,
  adults INTEGER,
  children INTEGER,
...
CREATE INDEX idx_guest_favorites_property_id ON guest_favorites(property_id);
...
CREATE TRIGGER update_property_locations_updated_at BEFORE UPDATE ON property_locations FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER update_property_floor_plans_updated_at BEFORE UPDATE ON property_floor_plans FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER update_property_guests_amount_updated_at BEFORE UPDATE ON property_guests_amount FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER update_property_amenities_updated_at BEFORE UPDATE ON property_amenities FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER update_property_price_details_updated_at BEFORE UPDATE ON property_price_details FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
ALTER TABLE property_guests_amount ENABLE ROW LEVEL SECURITY;
ALTER TABLE property_images ENABLE ROW LEVEL SECURITY;
ALTER TABLE amenities ENABLE ROW LEVEL SECURITY;
ALTER TABLE property_amenities ENABLE ROW LEVEL SECURITY;
ALTER TABLE property_price_details ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Public properties are viewable by everyone" ON properties FOR SELECT USING (status = 'PUBLISHED');
CREATE POLICY "Hosts can manage their own properties" ON properties FOR ALL USING (auth.uid()::text = host_id::text);

CREATE POLICY "Bookings are viewable by guest and host" ON bookings FOR SELECT USING (
  auth.uid()::text = guest_id::text OR 
  auth.uid()::text IN (SELECT host_id::text FROM properties WHERE id = property_id)
);
